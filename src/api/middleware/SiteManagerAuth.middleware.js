...
export const siteManager_auth = async (request, response, next) => {
	try {
		...
		if (secret) {
			if (request.headers.authorization && request.headers.authorization.startsWith("Bearer")) {
				...
				const siteManager = await SiteManagerModel.findOne({
					_id: decode,
					authToken: authToken,
				});
				...
				logger.info(`Authentication Token for ID ${siteManager._id} is Accepted`);
				next();
			} else {
				response.status(401);
